...
def test_frontend_function(
    ...
    **all_as_kwargs_np,
    ...
    if "/" in frontend and not framework_comparator(frontend):
        ...
        try:
            ...
            if ivy.isscalar(frontend_ret):
                ...
                frontend_ret = arrays_to_frontend(create_frontend_array)(frontend_ret)
                # tuplify the frontend return
                if not isinstance(frontend_ret, tuple):
                    frontend_ret = (frontend_ret,)
                frontend_ret_idxs = ivy.nested_argwhere(
                    frontend_ret,
                    lambda x: isinstance(x, np.ndarray) or _is_frontend_array(x),
                )
    ...
    ret_np_flat = flatten_frontend_to_np(ret=ret, frontend_array_fn=create_frontend_array)
